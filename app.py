...
db_connection = r.connect(RDB_HOST, RDB_PORT) #не использовать глобальные переменные
...
class event_res:
    ...
    def on_get(self, req, resp):
        ...
        resp.set_header('Access-Control-Allow-Origin','*') #реализовать на уровне nginx
    ...
    def on_delete(self, req, resp):
        ...
        try:
            result = json.loads(raw_json, encoding='utf-8')
            eventCursor = r.db(PROJECT_DB).table(PROJECT_TABLE).filter({'shedMsg': result['shedMsg'], 'shedTime': result['shedTime']}).delete().run(db_connection)#изменить на удаление через id
...
api = falcon.API()
api.add_route('/events', event_res())
db_setup()